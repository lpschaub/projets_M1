	...
	def build_voc(self) : 
		...
		for fic in self.corpus.lire():
			...
			for n in range(len(doc)):
				...
				if token not in self.voc and token.is_punct == False:
					self.voc.append(token)
				if n+1 < len(doc):
					bigram = (str(doc[n]), str(doc[n+1]))
					self.tokens.append(bigram)
					if bigram not in self.bigrams:
						self.bigrams.append(bigram)
				if n+2 < len(doc):
					trigram = (str(doc[n]), str(doc[n+1]), str(doc[n+2]))
					self.tokens.append(trigram)
					if trigram not in self.trigrams:
						self.trigrams.append(trigram)
		return (self.voc, self.bigrams, self.trigrams)
...
class Corpus(object) : 
	...
	def lire (self) :
		pd.read_csv(open(self.corpus), delimiter='\t')
...
class CorpusCSV(Corpus) : 
	...
	def getBOW(self) : 
		for fic in self.lire() : 
			...
			presence = []
			texte = fic
			texte = self.string(texte).replace('\n',' ')
			texte = texte.replace('(','')
			texte = texte.replace(')','')
        	...
        	for n in range(len(doc)):
        		token = str(doc[n])
				self.nv_voc.append(token)
				if n+1 < len(doc):
					bigram = (str(doc[n]), str(doc[n+1]))
					self.nv_vocbi.append(bigram)
				if n+2 < len(doc):
					trigram = (str(doc[n]), str(doc[n+1]), str(doc[n+2]))
					self.nv_voctri.append(trigram)
        	...
        	for token in self.voc:
            	if token in nv_voc:
            		...
            		presence.append(0)
			self.bow.append(presence)

			presencebi = []
			for bigram in self.bigrams:
				if bigram in nv_vocbi:
					presencebi.append(1)
				else:
					presence.append(0)
			self.bowbi.append(presencebi)

			presencetri = []
			for trigram in self.trigrams:
				if trigrams in nv_voctri:
					presencetri.append(1)
				else:
					presencetri.append(0)
			self.bowtri.append(presencetri)
		return(self.bow, self.bowbi, self.bowtri)
