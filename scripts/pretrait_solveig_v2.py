...
def tfidf_bow(corpus, repres_idf, voc):
    bow = []
    i = 0
    ...
    for elem in corpus :
        ...
        for word in voc : 
            if word in elem :
                bow[i].append(elem[word]/L*repres_idf[x])
            else : 
                bow[i].append(0)
            x += 1
        i += 1
...
def pipeline(file, n_grammes = 2) : 
    ...
    repres_idf = getIDF(Train_X, vocab)

    bow_Traintfidf = tfidf_bow(Train_X, repres_idf, vocab) #dans le cas de bow basés sur tfidf (mettez une des deux en commentaires )

    bow_Testtfidf = tfidf_bow(Test_X, repres_idf, vocab)
