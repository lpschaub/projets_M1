...
class Voc(object) : 
	...
	def build_voc(self) : 
		for fic in self.corpus.lire() : 
			ficstring = clean(self.corpus.string(fic))
			fictok = word_tokenize(ficstring)
			for i in fictok :
...
class Corpus(object) : 
	...
	def getBOW(self) : 
		for fic in self.lire() :
			is_in = []
			ficstring = clean(self.string(fic))
			doctok = word_tokenize(ficstring)
