...
class Voc(object) : 
	...
	def build_voc(self) : 
		for ligne in self.corpus.lire()['text']:
	...
	def build_voc_bi(self) : 
		for ligne in self.corpus.lire()['text']:
			ligne = self.clean(ligne)
			ligne = ligne.split()
			for token in ligne :
				if ligne.index(token) >= len(ligne)-1 :
					break
				elif (f"{token} {ligne[ligne.index(token)+1]}") in self.bi:
					continue
				elif token in ['!',',','.','?','>','<','/',':','(',')','<','>','...','``',"'"]:
					continue
				else:
					self.bi.append(f"{token} {ligne[ligne.index(token)+1]}")
		return self.bi
    
	def build_voc_tri(self) : 
		for ligne in self.corpus.lire()['text']:
			ligne = self.clean(ligne)
			ligne = ligne.split()
			for token in ligne :
				if ligne.index(token) >= len(ligne)-2 :
					break
				elif (f"{token} {ligne[ligne.index(token)+1]} {ligne[ligne.index(token)+2]}") in self.tri:
					continue
				elif token in ['!',',','.','?','>','<','/',':','(',')','<','>','...','``',"'"]:
					continue
				else:
					self.tri.append(f"{token} {ligne[ligne.index(token)+1]} {ligne[ligne.index(token)+2]}")
		return self.tri
	...
	def load_bi(self, vocfile) : 

		self.bi = Voc.string(vocfile)

	def load_tri(self, vocfile) : 

		self.tri = Voc.string(vocfile)


class Corpus(object) : 

	def __init__(self, path, voc = "", bi = "", tri = "") : 
	...
	def lire (self) :
		return pd.read_csv(self.corpus, sep='\t')
	...
	def getBOW_bi(self) : 
		...
		for ligne in self.lire()['text']:
			...
			for token in ligne :
				if ligne.index(token) >= len(ligne)-1 :
					break
				ligne_bi.append((f"{token} {ligne[ligne.index(token)+1]}"))
			
			for token in self.bi :
				if token in ligne_bi:
					liste.append(1)
				else:
					liste.append(0)

			self.bow_bi.append(liste)
		return self.bow_bi
	...
	def getBOW_tri(self) : 
		...
		for ligne in self.lire()['text']:
			...
			for token in ligne :
				if ligne.index(token) >= len(ligne)-2 :
					break
				ligne_tri.append(f"{token} {ligne[ligne.index(token)+1]} {ligne[ligne.index(token)+2]}")
			
			for token in self.tri :
				if token in ligne_tri:
					liste.append(1)
				else:
					liste.append(0)

			self.bow_tri.append(liste)
		return self.bow_tri
