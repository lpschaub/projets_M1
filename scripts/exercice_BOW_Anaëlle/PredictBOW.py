...
class Voc(object) : 
	...
	def build_voc(self) : 
		for fic in self.corpus.lire() :
			ficstring = self.clean(self.corpus.string(fic))
			for token in ficstring.split():
...
class Corpus(object) : 
	...
	def getBOW(self) : 
		...
		for fic in self.lire():
			liste = []
			ficstring = self.clean(self.string(fic))
			ficstring = ficstring.split()
			
			for token in self.voc :
				if token in ficstring:
					liste.append(1)
				else:
					liste.append(0)
