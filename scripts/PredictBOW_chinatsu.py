...
class Voc(object) : 
	...
	def build_voc(self) : 
		...
		for fic in self.corpus.lire() : 
			...
			ficstring = p.split(ficstring)
			for mots in ficstring :
				if mots.lower() not in self.voc :
					self.voc.append(mots.lower())
...
class Corpus(object) : 
	...
	def getBOW(self) : 
		...
		for fic in self.lire():
			...
			ficstring = p.split(ficstring.lower())
			temp = []
			for i in range(len(self.voc)) :
				if self.voc[i] in ficstring :
					temp.append(1)
				else:
					temp.append(0)
